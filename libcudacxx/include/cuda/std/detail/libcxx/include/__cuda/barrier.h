...
struct aligned_size_t
{
  ...
  _LIBCUDACXX_INLINE_VISIBILITY explicit aligned_size_t(size_t __s)
      : value(__s)
  {}
...
enum async_contract_fulfillment { none, async };
...
public:
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr barrier(
      _CUDA_VSTD::ptrdiff_t __expected, _CompletionF __completion = _CompletionF())
      : _CUDA_VSTD::__barrier_base<_CompletionF, _Sco>(__expected, __completion)
  {}

  _LIBCUDACXX_INLINE_VISIBILITY friend void
  init(barrier* __b, _CUDA_VSTD::ptrdiff_t __expected)
  {
...
class barrier<thread_scope_block, _CUDA_VSTD::__empty_completion> : public __block_scope_barrier_base
{
  ...
  _LIBCUDACXX_DEVICE
  friend inline _CUDA_VSTD::uint64_t* device::_LIBCUDACXX_ABI_NAMESPACE::barrier_native_handle(
...
public:
  ...
  _LIBCUDACXX_INLINE_VISIBILITY
  barrier(
      _CUDA_VSTD::ptrdiff_t __expected, _CUDA_VSTD::__empty_completion __completion = _CUDA_VSTD::__empty_completion())
  {
    static_assert(
        _LIBCUDACXX_OFFSET_IS_ZERO(barrier<thread_scope_block>, __barrier), "fatal error: bad barrier layout");
    ...
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_90,
        (
            if (__isShared(&__barrier)) {
              asm volatile("mbarrier.inval.shared.b64 [%0];" ::"r"(
  ...
  _LIBCUDACXX_NODISCARD_ATTRIBUTE _LIBCUDACXX_INLINE_VISIBILITY arrival_token
  arrive(_CUDA_VSTD::ptrdiff_t __update = 1)
  {
    ...
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_90,
        ...
        (
            ...
            __syncwarp(__mask);
            if (__leader == __laneid) { __token = __barrier.arrive(__inc); } __token =
                __shfl_sync(__active, __token, __leader);),
        NV_IS_HOST,
...
private:
  _LIBCUDACXX_INLINE_VISIBILITY inline bool
  __test_wait_sm_80(arrival_token __token) const
  {
    int32_t __ready = 0;
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_80,
        (asm volatile(
            "{\n\t"
            ".reg .pred p;\n\t"
  ...
  __try_wait(arrival_token __token) const
  {
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_90,
        (
  ...
  // Document de drop > uint32_t for __nanosec on public for APIs
  _LIBCUDACXX_INLINE_VISIBILITY bool
  __try_wait(arrival_token __token, _CUDA_VSTD::chrono::nanoseconds __nanosec) const
  {
    ...
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_90,
        (
            ...
            do {
              const _CUDA_VSTD::uint32_t __wait_nsec =
                  static_cast<_CUDA_VSTD::uint32_t>((__nanosec - __elapsed).count());
              asm volatile(
                  "{\n\t"
                  ".reg .pred p;\n\t"
        ...
        NV_PROVIDES_SM_80,
        (
            bool __ready = 0; if (!__isShared(&__barrier)) {
              return _CUDA_VSTD::__libcpp_thread_poll_with_backoff(
                  _CUDA_VSTD::__barrier_poll_tester_phase<barrier>(this, _CUDA_VSTD::move(__token)), __nanosec);
            }
  ...
  _LIBCUDACXX_INLINE_VISIBILITY inline bool
  __test_wait_parity_sm_80(bool __phase_parity) const
  {
    uint16_t __ready = 0;
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_80,
        (asm volatile(
  ...
  _LIBCUDACXX_INLINE_VISIBILITY bool
  __try_wait_parity(bool __phase_parity) const
  {
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_90,
        (
            if (!__isClusterShared(&__barrier)) {
...
public:
  ...
  _LIBCUDACXX_INLINE_VISIBILITY void
  wait_parity(bool __phase_parity) const
  {
    _CUDA_VSTD::__libcpp_thread_poll_with_backoff(
  ...
  _LIBCUDACXX_INLINE_VISIBILITY static constexpr ptrdiff_t
  max() noexcept
  {
    return (1 << 20) - 1;
  }
...
_LIBCUDACXX_DEVICE
inline _CUDA_VSTD::uint64_t*
barrier_native_handle(barrier<thread_scope_block>& b)
{
...
_LIBCUDACXX_NODISCARD_ATTRIBUTE _LIBCUDACXX_DEVICE inline barrier<thread_scope_block>::arrival_token
barrier_arrive_tx(barrier<thread_scope_block>& __b, _CUDA_VSTD::ptrdiff_t __arrive_count_update,
    _CUDA_VSTD::ptrdiff_t __transaction_count_update)
{
  ...
  NV_IF_TARGET(
      ...
      (
          ...
          auto __bh = __cvta_generic_to_shared(barrier_native_handle(__b)); if (__arrive_count_update == 1) {
            ...
            asm("mbarrier.expect_tx.relaxed.cta.shared::cta.b64 [%0], %1;"
                ...
                "r"(static_cast<_CUDA_VSTD::uint32_t>(__transaction_count_update))
                : "memory");
            asm("mbarrier.arrive.release.cta.shared::cta.b64 %0, [%1], %2;"
                : "=l"(__token)
                : "r"(static_cast<_CUDA_VSTD::uint32_t>(__bh)),
...
template <_CUDA_VSTD::size_t _Alignment>
_LIBCUDACXX_INLINE_VISIBILITY inline void
__strided_memcpy(char* __destination, char const* __source, _CUDA_VSTD::size_t __total_size, _CUDA_VSTD::size_t __rank,
    _CUDA_VSTD::size_t __stride = 1)
{
  if (__stride == 1) {
    memcpy(__destination, __source, __total_size);
  } else {
    for (_CUDA_VSTD::size_t __offset = __rank * _Alignment; __offset < __total_size;
         __offset += __stride * _Alignment) {
...
{
  _LIBCUDACXX_DEVICE static inline async_contract_fulfillment
  __copy(char* __destination, char const* __source, _CUDA_VSTD::size_t __total_size, _CUDA_VSTD::size_t __rank,
      _CUDA_VSTD::size_t __stride)
  {
      ...
      _CUDA_VSTD::size_t __stride)
  {
    ...
    NV_IF_TARGET(NV_IS_DEVICE,
        (for (_CUDA_VSTD::size_t __offset = __rank * 4; __offset < __total_size; __offset += __stride * 4) {
          asm volatile("cp.async.ca.shared.global [%0], [%1], 4, 4;" ::"r"(
                           static_cast<_CUDA_VSTD::uint32_t>(__cvta_generic_to_shared(__destination + __offset))),
                       "l"(__source + __offset)
                       : "memory");
        }))
      ...
      _CUDA_VSTD::size_t __stride)
  {
    ...
    NV_IF_TARGET(NV_IS_DEVICE,
        (for (_CUDA_VSTD::size_t __offset = __rank * 8; __offset < __total_size; __offset += __stride * 8) {
          asm volatile("cp.async.ca.shared.global [%0], [%1], 8, 8;" ::"r"(
                           static_cast<_CUDA_VSTD::uint32_t>(__cvta_generic_to_shared(__destination + __offset))),
                       "l"(__source + __offset)
                       : "memory");
        }))
...
struct __memcpy_arrive_on_impl
{
  ...
  _LIBCUDACXX_INLINE_VISIBILITY static inline void
  __arrive_on(barrier<_Sco, _CompF>& __barrier, async_contract_fulfillment __is_async)
  {
    NV_DISPATCH_TARGET(NV_PROVIDES_SM_90,
        (if (_Is_mbarrier && __isClusterShared(&__barrier) && !__isShared(&__barrier)) { __trap(); }))

    NV_DISPATCH_TARGET(NV_PROVIDES_SM_80, (if (__is_async == async_contract_fulfillment::async) {
  ...
  _LIBCUDACXX_INLINE_VISIBILITY static inline void
  __arrive_on(pipeline<_Sco>& __pipeline, async_contract_fulfillment __is_async)
  {
...
_LIBCUDACXX_INLINE_VISIBILITY void inline __memcpy_async_sm_dispatch(_Group const& __group, char* __destination,
    char const* __source, _CUDA_VSTD::size_t __size, _Sync& __sync, async_contract_fulfillment& __is_async)
{
  // Broken out of __memcpy_async to avoid nesting dispatches
  NV_DISPATCH_TARGET(
      NV_PROVIDES_SM_80,
      __is_async =
          __memcpy_async_impl<16>::__copy(__destination, __source, __size, __group.thread_rank(), __group.size());)
}

template <_CUDA_VSTD::size_t _Native_alignment, typename _Group, typename _Sync>
_LIBCUDACXX_INLINE_VISIBILITY async_contract_fulfillment inline __memcpy_async(
    _Group const& __group, char* __destination, char const* __source, _CUDA_VSTD::size_t __size, _Sync& __sync)
{
  ...
  NV_DISPATCH_TARGET(
      ...
      if (__isShared(__destination) && __isGlobal(__source)) {
        if (_Native_alignment < 4) {
          ...
          case 4:
            __is_async =
                __memcpy_async_impl<8>::__copy(__destination, __source, __size, __group.thread_rank(), __group.size());
            break;
          case 3:
            __is_async =
                __memcpy_async_impl<4>::__copy(__destination, __source, __size, __group.thread_rank(), __group.size());
            break;
          case 2: // fallthrough
          case 1:
            __is_async =
                __memcpy_async_impl<1>::__copy(__destination, __source, __size, __group.thread_rank(), __group.size());
            break;
          }
        } else {
          __is_async = __memcpy_async_impl<_Native_alignment>::__copy(
              __destination, __source, __size, __group.thread_rank(), __group.size());
        }
      } else {
...
struct __single_thread_group
{
  ...
  sync() const
  {}
  _LIBCUDACXX_INLINE_VISIBILITY constexpr _CUDA_VSTD::size_t
  size() const
  {
    return 1;
  };
...
memcpy_async(_Group const& __group, _Tp* __destination, _Tp const* __source, _CUDA_VSTD::size_t __size,
    barrier<_Sco, _CompF>& __barrier)
{
  ...
  return __memcpy_async<alignof(_Tp)>(
...
template <typename _Group, class _Tp, _CUDA_VSTD::size_t _Alignment, thread_scope _Sco, typename _CompF,
    _CUDA_VSTD::size_t _Larger_alignment = (alignof(_Tp) > _Alignment) ? alignof(_Tp) : _Alignment>
_LIBCUDACXX_INLINE_VISIBILITY async_contract_fulfillment
    ...
    barrier<_Sco, _CompF>& __barrier)
{
  ...
  return __memcpy_async<_Larger_alignment>(
      __group, reinterpret_cast<char*>(__destination), reinterpret_cast<char const*>(__source), __size, __barrier);
}
...
_LIBCUDACXX_INLINE_VISIBILITY async_contract_fulfillment
memcpy_async(_Group const& __group, void* __destination, void const* __source, _CUDA_VSTD::size_t __size,
    barrier<_Sco, _CompF>& __barrier)
{
  return __memcpy_async<1>(
      __group, reinterpret_cast<char*>(__destination), reinterpret_cast<char const*>(__source), __size, __barrier);
}
...
memcpy_async(_Group const& __group, void* __destination, void const* __source, aligned_size_t<_Alignment> __size,
    barrier<_Sco, _CompF>& __barrier)
{
  return __memcpy_async<_Alignment>(
      __group, reinterpret_cast<char*>(__destination), reinterpret_cast<char const*>(__source), __size, __barrier);
}
