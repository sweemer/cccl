...
private:
  ...
  _LIBCUDACXX_INLINE_VISIBILITY static true_type __test(__void_t<typename _Up::iterator_category>* = nullptr,
      __void_t<typename _Up::difference_type>* = nullptr, __void_t<typename _Up::value_type>* = nullptr,
      __void_t<typename _Up::reference>* = nullptr, __void_t<typename _Up::pointer>* = nullptr);
...
namespace __iterator_traits_detail {
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__cpp17_iterator_,
    requires(_Ip __i)( //
        requires(__can_reference<decltype(*__i)>), requires(same_as<_Ip&, decltype(++__i)>),
        requires(__can_reference<decltype(*__i++)>), requires(copyable<_Ip>)));

template <class _Ip>
_LIBCUDACXX_CONCEPT __cpp17_iterator = _LIBCUDACXX_FRAGMENT(__cpp17_iterator_, _Ip);

template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__cpp17_input_iterator_,
    requires(_Ip __i)(
        typename(common_reference_t<iter_reference_t<_Ip>&&, typename indirectly_readable_traits<_Ip>::value_type&>),
        typename(common_reference_t<decltype(*__i++)&&, typename indirectly_readable_traits<_Ip>::value_type&>),
        requires(__cpp17_iterator<_Ip>), requires(equality_comparable<_Ip>),
...
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__cpp17_forward_iterator_,
    requires(_Ip __i)(requires(__cpp17_input_iterator<_Ip>), requires(convertible_to<decltype(__i++), _Ip const&>),
        requires(same_as<iter_reference_t<_Ip>, decltype(*__i++)>), requires(constructible_from<_Ip>),
...
_LIBCUDACXX_CONCEPT_FRAGMENT(__cpp17_bidirectional_iterator_,
    requires(_Ip __i)(requires(__cpp17_forward_iterator<_Ip>), requires(same_as<_Ip&, decltype(--__i)>),
...
_LIBCUDACXX_CONCEPT_FRAGMENT(__cpp17_random_access_iterator_,
    requires(_Ip __i, typename incrementable_traits<_Ip>::difference_type __n)( //
        requires(same_as<_Ip&, decltype(__i += __n)>), requires(same_as<_Ip&, decltype(__i -= __n)>),
        requires(same_as<_Ip, decltype(__i + __n)>), requires(same_as<_Ip, decltype(__n + __i)>),
        requires(same_as<_Ip, decltype(__i - __n)>), requires(same_as<decltype(__n), decltype(__i - __i)>),
...
// `forward_iterator_tag` if `I` satisfies `cpp17-forward-iterator`, or otherwise
template <class _Ip>
struct __deduce_iterator_category<_Ip, enable_if_t< !__iterator_traits_detail::__cpp17_bidirectional_iterator<_Ip>
...
struct __is_exactly_cpp17_input_iterator
    : public integral_constant<bool, __has_iterator_category_convertible_to<_Tp, input_iterator_tag>::value
