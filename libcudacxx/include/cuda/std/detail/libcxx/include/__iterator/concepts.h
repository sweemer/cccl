...
// [iterator.concept.readable]
template <class _In>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_readable_impl_,
    requires(const _In __i)(typename(iter_value_t<_In>), typename(iter_reference_t<_In>),
        typename(iter_rvalue_reference_t<_In>), requires(same_as<iter_reference_t<_In>, decltype(*__i)>),
...
// [iterator.concept.writable]
template <class _Out, class _Tp>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_writable_,
    requires(_Out&& __o, _Tp&& __t)(typename(decltype(*__o = _CUDA_VSTD::forward<_Tp>(__t))),
        ...
        typename(decltype(const_cast<const iter_reference_t<_Out>&&>(
                              *_CUDA_VSTD::forward<_Out>(__o)) = _CUDA_VSTD::forward<_Tp>(__t)))));
...
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__weakly_incrementable_,
    requires(_Ip __i)(typename(iter_difference_t<_Ip>), requires(!same_as<_Ip, bool>), requires(movable<_Ip>),
        requires(__signed_integer_like<iter_difference_t<_Ip>>), requires(same_as<_Ip&, decltype(++__i)>), (__i++)));
...
// [iterator.concept.inc]
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__incrementable_,
    requires(_Ip __i)(
...
// [iterator.concept.iterator]
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__input_or_output_iterator_,
...
// [iterator.concept.sentinel]
template <class _Sp, class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__sentinel_for_,
    requires()(requires(semiregular<_Sp>), requires(input_or_output_iterator<_Ip>),
...
template <class _Sp, class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__sized_sentinel_for_,
    requires(const _Ip& __i, const _Sp& __s)(requires(sentinel_for<_Sp, _Ip>),
...
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__input_iterator_,
    requires()(requires(input_or_output_iterator<_Ip>), requires(indirectly_readable<_Ip>),
        typename(_ITER_CONCEPT<_Ip>), requires(derived_from<_ITER_CONCEPT<_Ip>, input_iterator_tag>)));
...
// [iterator.concept.output]
template <class _Ip, class _Tp>
_LIBCUDACXX_CONCEPT_FRAGMENT(__output_iterator_,
    requires(_Ip __it, _Tp&& __t)(requires(input_or_output_iterator<_Ip>), requires(indirectly_writable<_Ip, _Tp>),
...
// [iterator.concept.forward]
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__forward_iterator_,
    requires()(requires(input_iterator<_Ip>), requires(derived_from<_ITER_CONCEPT<_Ip>, forward_iterator_tag>),
        requires(incrementable<_Ip>), requires(sentinel_for<_Ip, _Ip>)));
...
// [iterator.concept.bidir]
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__bidirectional_iterator_,
    requires(_Ip __i)(requires(forward_iterator<_Ip>),
        requires(derived_from<_ITER_CONCEPT<_Ip>, bidirectional_iterator_tag>),
        requires(same_as<_Ip&, decltype(--__i)>), requires(same_as<_Ip, decltype(__i--)>)));
...
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__random_access_iterator_operations_,
    requires(_Ip __i, const _Ip __j, const iter_difference_t<_Ip> __n)(requires(same_as<_Ip&, decltype(__i += __n)>),
        requires(same_as<_Ip, decltype(__j + __n)>), requires(same_as<_Ip, decltype(__n + __j)>),
        requires(same_as<_Ip&, decltype(__i -= __n)>), requires(same_as<_Ip, decltype(__j - __n)>),
...
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__random_access_iterator_,
    requires()(requires(bidirectional_iterator<_Ip>),
        requires(derived_from<_ITER_CONCEPT<_Ip>, random_access_iterator_tag>), requires(totally_ordered<_Ip>),
        requires(sized_sentinel_for<_Ip, _Ip>), requires(__random_access_iterator_operations<_Ip>)));
...
template <class _Ip>
_LIBCUDACXX_CONCEPT_FRAGMENT(__contiguous_iterator_,
    requires(const _Ip& __i)(requires(random_access_iterator<_Ip>),
...
// [indirectcallable.indirectinvocable]
template <class _Fp, class _It>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_unary_invocable,
    requires()(requires(indirectly_readable<_It>), requires(copy_constructible<_Fp>),
        requires(invocable<_Fp&, iter_value_t<_It>&>), requires(invocable<_Fp&, iter_reference_t<_It>>),
...
template <class _Fp, class _It>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_regular_unary_invocable_,
    requires()(requires(indirectly_readable<_It>), requires(copy_constructible<_Fp>),
        requires(regular_invocable<_Fp&, iter_value_t<_It>&>), requires(regular_invocable<_Fp&, iter_reference_t<_It>>),
...
template <class _Fp, class _It>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirect_unary_predicate_,
    requires()(requires(indirectly_readable<_It>), requires(copy_constructible<_Fp>),
        requires(predicate<_Fp&, iter_value_t<_It>&>), requires(predicate<_Fp&, iter_reference_t<_It>>),
...
template <class _Fp, class _It1, class _It2>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirect_binary_predicate_,
    requires()(requires(indirectly_readable<_It1>), requires(indirectly_readable<_It2>),
        requires(copy_constructible<_Fp>), requires(predicate<_Fp&, iter_value_t<_It1>&, iter_value_t<_It2>&>),
...
template <class _Fp, class _It1, class _It2>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirect_equivalence_relation_,
    requires()(requires(indirectly_readable<_It1>), requires(indirectly_readable<_It2>),
...
template <class _Fp, class _It1, class _It2>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirect_strict_weak_order_,
    requires()(requires(indirectly_readable<_It1>), requires(indirectly_readable<_It2>),
        requires(copy_constructible<_Fp>), requires(strict_weak_order<_Fp&, iter_value_t<_It1>&, iter_value_t<_It2>&>),
...
template <class _In, class _Out>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_movable_,
...
template <class _In, class _Out>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_movable_storable_,
    requires()(requires(indirectly_movable<_In, _Out>), requires(indirectly_writable<_Out, iter_value_t<_In>>),
...
template <class _In, class _Out>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_copyable_,
...
template <class _In, class _Out>
_LIBCUDACXX_CONCEPT_FRAGMENT(__indirectly_copyable_storable_,
    requires()(requires(indirectly_copyable<_In, _Out>), requires(indirectly_writable<_Out, iter_value_t<_In>&>),
        requires(indirectly_writable<_Out, const iter_value_t<_In>&>),
        requires(indirectly_writable<_Out, iter_value_t<_In>&&>),
        requires(indirectly_writable<_Out, const iter_value_t<_In>&&>), requires(copyable<iter_value_t<_In>>),
