...
_LIBCUDACXX_CONCEPT_FRAGMENT(__exchangeable_,
    requires()(requires(!__unqualified_swappable_with<_Tp&, _Tp&>), requires(move_constructible<_Tp>),
...
struct __fn
{
  ...
  _LIBCUDACXX_INLINE_VISIBILITY constexpr void
  operator()(_Tp (&__t)[_Size], _Up (&__u)[_Size]) const noexcept(__noexcept_swappable_arrays<_Tp, _Up>)
  {
...
_LIBCUDACXX_CONCEPT_FRAGMENT(__swappable_arrays_,
    requires(_Tp (&__t)[_Size::value], _Up (&__u)[_Size::value], const __fn& __swap)(
        ...
        requires(_LIBCUDACXX_TRAIT(extent, _Tp) == _LIBCUDACXX_TRAIT(extent, _Up)), (__swap(__t[0], __u[0]))));
...
template <class _Tp, class _Up>
_LIBCUDACXX_CONCEPT_FRAGMENT(__swappable_with_,
    requires(_Tp&& __t, _Up&& __u)(requires(common_reference_with<_Tp, _Up>),
        (_CUDA_VRANGES::swap(_CUDA_VSTD::forward<_Tp>(__t), _CUDA_VSTD::forward<_Tp>(__t))),
        (_CUDA_VRANGES::swap(_CUDA_VSTD::forward<_Up>(__u), _CUDA_VSTD::forward<_Up>(__u))),
        (_CUDA_VRANGES::swap(_CUDA_VSTD::forward<_Tp>(__t), _CUDA_VSTD::forward<_Up>(__u))),
        (_CUDA_VRANGES::swap(_CUDA_VSTD::forward<_Up>(__u), _CUDA_VSTD::forward<_Tp>(__t)))));
