...
struct async_resource
{
  ...
  void
  deallocate(void* ptr, std::size_t, std::size_t)
  {}

  void*
  allocate_async(std::size_t, std::size_t, cuda::stream_ref)
  {
    return nullptr;
  }
  ...
  bool
  operator==(const async_resource& other) const
  {
    return true;
  }
  bool
  ...
  friend void
  get_property(const async_resource&, Property) noexcept
  {}
...
static_assert(
    sizeof(cuda::mr::async_resource_ref<property_with_value<short>, property_with_value<int> >) == (4 * sizeof(void*)),
    "");
static_assert(sizeof(cuda::mr::async_resource_ref<property_with_value<short>, property_without_value<int> >)
                  == (3 * sizeof(void*)),
    "");
static_assert(sizeof(cuda::mr::async_resource_ref<property_without_value<short>, property_with_value<int> >)
                  == (3 * sizeof(void*)),
    "");
...
int
InvokeIfWithValue(const Ref& ref)
{
  return -1;
}
...
_LIBCUDACXX_REQUIRES(cuda::property_with_value<Property>) //
int
InvokeIfWithoutValue(const Ref& ref)
{
  return -1;
}
...
{
  using res = async_resource<property_with_value<short>, property_with_value<int> >;
