...
struct storage
{
  ...
  operator==(const storage& lhs, const storage& rhs)
  {
    for (cuda::std::size_t i = 0; i < size; ++i) {
  ...
  __host__ __device__ friend bool
  operator==(const storage& lhs, const T& rhs)
  {
    for (cuda::std::size_t i = 0; i < size; ++i) {
      if (lhs.data[i] != rhs + i) {
        return false;
...
template <class T, template <typename, typename> class SourceSelector, template <typename, typename> class DestSelector,
    template <typename, typename> class BarrierSelector, cuda::thread_scope BarrierScope, typename... CompletionF >
