...
compute(int* global_out, int const* shared_in)
{
...
__global__ void
with_staging(int* global_out, int const* global_in, size_t size, size_t batch_sz)
{
  ...
  cuda::memcpy_async(
      block, shared + shared_offset[0], global_in + block_batch(0), sizeof(int) * block.size(), pipeline);
  ...
  for (size_t batch = 1; batch < batch_sz; ++batch) {
    ...
    cuda::memcpy_async(block,
        shared + shared_offset[copy_stage_idx],
        global_in + global_copy_idx,
