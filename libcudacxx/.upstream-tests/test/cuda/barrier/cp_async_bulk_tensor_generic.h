...
tensor_len(std::initializer_list<int> dims)
{
  int len = 1;
  for (int d : dims) {
    len *= d;
...
smem_lin_idx_to_gmem_lin_idx(int smem_lin_idx, std::initializer_list<int> smem_coord,
    std::initializer_list<int> smem_dims, std::initializer_list<int> gmem_dims)
{
...
cp_tensor_global_to_shared(CUtensorMap* tensor_map, std::initializer_list<int> indices, void* smem, barrier& bar)
{
  const int* idxs = indices.begin();

  switch (indices.size()) {
  case 1:
    cde::cp_async_bulk_tensor_1d_global_to_shared(smem, tensor_map, idxs[0], bar);
    break;
  case 2:
    cde::cp_async_bulk_tensor_2d_global_to_shared(smem, tensor_map, idxs[0], idxs[1], bar);
    break;
  case 3:
    cde::cp_async_bulk_tensor_3d_global_to_shared(smem, tensor_map, idxs[0], idxs[1], idxs[2], bar);
    break;
  case 4:
    cde::cp_async_bulk_tensor_4d_global_to_shared(smem, tensor_map, idxs[0], idxs[1], idxs[2], idxs[3], bar);
    break;
  case 5:
    cde::cp_async_bulk_tensor_5d_global_to_shared(smem, tensor_map, idxs[0], idxs[1], idxs[2], idxs[3], idxs[4], bar);
    break;
  default:
...
__device__ inline void
cp_tensor_shared_to_global(CUtensorMap* tensor_map, std::initializer_list<int> indices, void* smem)
{
  const int* idxs = indices.begin();

  switch (indices.size()) {
  case 1:
    cde::cp_async_bulk_tensor_1d_shared_to_global(tensor_map, idxs[0], smem);
    break;
  case 2:
    cde::cp_async_bulk_tensor_2d_shared_to_global(tensor_map, idxs[0], idxs[1], smem);
    break;
  case 3:
    cde::cp_async_bulk_tensor_3d_shared_to_global(tensor_map, idxs[0], idxs[1], idxs[2], smem);
    break;
  case 4:
    cde::cp_async_bulk_tensor_4d_shared_to_global(tensor_map, idxs[0], idxs[1], idxs[2], idxs[3], smem);
    break;
  case 5:
    cde::cp_async_bulk_tensor_5d_shared_to_global(tensor_map, idxs[0], idxs[1], idxs[2], idxs[3], idxs[4], smem);
    break;
  default:
    assert(false && "Wrong number of dimensions.");
  }
...
__device__ void
test(std::initializer_list<int> smem_coord, std::initializer_list<int> smem_dims, std::initializer_list<int> gmem_dims,
...
template <typename T>
CUtensorMap
map_encode(T* tensor_ptr, std::initializer_list<int> gmem_dims, std::initializer_list<int> smem_dims)
{
  ...
  CUresult res = cuTensorMapEncodeTiled(&tensor_map, // CUtensorMap *tensorMap,
...
template <typename T>
void
init_tensor_map(const T& gmem_tensor_symbol, std::initializer_list<int> gmem_dims, std::initializer_list<int> smem_dims)
{
