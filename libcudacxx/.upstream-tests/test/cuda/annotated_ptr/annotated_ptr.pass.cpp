...
static_assert(alignof(cuda::annotated_ptr<int, cuda::access_property::streaming>) == alignof(int*),
    "annotated_ptr must align with int*");
static_assert(
    alignof(cuda::annotated_ptr<int, cuda::access_property>) == alignof(int*), "annotated_ptr must align with int*");
...
__device__ __host__ __noinline__ void
test_read_access(In i, T* r)
{
  assert(i);
  assert(i - i == 0);
  assert((bool) i);
