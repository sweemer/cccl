...
inline constexpr bool
    check_has_difference_type<T, cuda::std::void_t<typename cuda::std::incrementable_traits<T>::difference_type>> =
        true;

template <class T, class Expected>
_LIBCUDACXX_CONCEPT_FRAGMENT(check_difference_type_matches_,
...
{
  __host__ __device__ friend unsigned int operator-(
      unsigned_int_subtraction_with_cv const&, unsigned_int_subtraction_with_cv const&);
  __host__ __device__ friend unsigned int operator-(
      unsigned_int_subtraction_with_cv const volatile&, unsigned_int_subtraction_with_cv const volatile&);
};
