...
public:
  ...
  __host__ __device__ constexpr unqualified_lookup_wrapper&
  operator++() noexcept
  {
    ++base_;
    return *this;
  ...
  operator==(unqualified_lookup_wrapper const& other) const noexcept
  {
  ...
  iter_move(unqualified_lookup_wrapper& i) noexcept(false)
  {
...
__host__ __device__ constexpr unscoped_enum
iter_move(unscoped_enum& e) noexcept(false)
{
  return e;
}

enum class scoped_enum { a, b, c };
__host__ __device__ constexpr scoped_enum*
iter_move(scoped_enum&) noexcept
{
  return nullptr;
}
...
iter_move(some_union& u) noexcept(false)
{
  return u.x;
}
...
public:
  ...
  __host__ __device__ constexpr move_tracker&
  operator=(move_tracker&& other) noexcept
  {
    moves_       = other.moves_ + 1;
    other.moves_ = 0;
    return *this;
