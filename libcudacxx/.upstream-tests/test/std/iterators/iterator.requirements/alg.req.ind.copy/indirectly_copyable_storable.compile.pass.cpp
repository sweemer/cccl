...
#if TEST_STD_VER > 17 \
    || !defined(TEST_COMPILER_MSVC) //  multiple versions of a defaulted special member functions are not allowed
...
struct cuda::std::basic_common_reference<NotConstructibleFromRefIn::ValueType, NotConstructibleFromRefIn::ReferenceType,
    X, Y>
{
...
template <template <class> class X, template <class> class Y>
struct cuda::std::basic_common_reference<NotConstructibleFromRefIn::ReferenceType, NotConstructibleFromRefIn::ValueType,
    X, Y>
{
  using type = CommonType&;
};
