...
{
  {
    ...
    typedef T::difference_type
        DT; // expected-error-re {{no type named 'difference_type' in 'cuda::std::{{.+}}::iterator_traits<{{.+}}>}}
    typedef T::value_type
        VT; // expected-error-re {{no type named 'value_type' in 'cuda::std::{{.+}}::iterator_traits<{{.+}}>}}
    typedef T::pointer
        PT; // expected-error-re {{no type named 'pointer' in 'cuda::std::{{.+}}::iterator_traits<{{.+}}>}}
    typedef T::reference
        RT; // expected-error-re {{no type named 'reference' in 'cuda::std::{{.+}}::iterator_traits<{{.+}}>}}
    typedef T::iterator_category
        CT; // expected-error-re {{no type named 'iterator_category' in 'cuda::std::{{.+}}::iterator_traits<{{.+}}>}}
  }
