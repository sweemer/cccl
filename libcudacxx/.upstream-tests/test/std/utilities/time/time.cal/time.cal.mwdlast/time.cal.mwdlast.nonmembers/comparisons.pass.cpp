...
main(int, char**)
{
  ...
  static_assert(testComparisons2(month_weekday_last{cuda::std::chrono::January, weekday_last{Tuesday}},
  ...
  for (unsigned i = 1; i < 12; ++i) {
    for (unsigned j = 1; j < 12; ++j) {
      assert((testComparisons2(month_weekday_last{month{i}, weekday_last{Tuesday}},
  ...
  for (unsigned i = 0; i < 6; ++i) {
    for (unsigned j = 0; j < 6; ++j) {
      assert((testComparisons2(month_weekday_last{January, weekday_last{weekday{i}}},
  ...
  //  both different
  assert((testComparisons2(month_weekday_last{month{1}, weekday_last{weekday{1}}},
