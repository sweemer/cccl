...
main(int, char**)
{
  ...
  static_assert(testComparisons2(month_weekday{cuda::std::chrono::January, weekday_indexed{Sunday, 1}},
  ...
  static_assert(testComparisons2(month_weekday{cuda::std::chrono::January, weekday_indexed{Monday, 1}},
                    month_weekday{cuda::std::chrono::January, weekday_indexed{Sunday, 2}},
                    false),
      "");
  ...
  for (unsigned i = 1; i < 12; ++i) {
    for (unsigned j = 1; j < 12; ++j) {
      assert((testComparisons2(month_weekday{month{i}, weekday_indexed{Sunday, 1}},
  ...
  for (unsigned i = 1; i < 5; ++i) {
    for (unsigned j = 1; j < 5; ++j) {
      assert((testComparisons2(month_weekday{month{2}, weekday_indexed{Sunday, i}},
  ...
  for (unsigned i = 0; i < 6; ++i) {
    for (unsigned j = 0; j < 6; ++j) {
      assert((testComparisons2(month_weekday{month{2}, weekday_indexed{weekday{i}, 2}},
