...
__host__ __device__ inline constexpr CallType
operator|(CallType LHS, CallType RHS)
{
...
struct EvilBool
{
  ...
  __host__ __device__ friend EvilBool
...
private:
  ...
  __host__ __device__ explicit ExplicitBool(bool x)
      : value(x)
  {}
  __host__ __device__ ExplicitBool&
  operator=(bool x)
  {
    value = x;
    return *this;
...
struct NoExceptEvilBool
{
  ...
  __host__ __device__ friend NoExceptEvilBool
  operator!(NoExceptEvilBool const& other) noexcept
  {
...
__host__ __device__ void call_operator_sfinae_test(){{// wrong number of arguments
