...
{
  __host__ __device__ constexpr ImplicitThrow(int x)
  {
    if (x != -1) {
      TEST_THROW(6);
    }
...
{
  __host__ __device__ constexpr explicit ExplicitThrow(int x)
  {
    if (x != -1) {
      TEST_THROW(6);
    }
...
implicit_conversion(optional<To>&& opt, const From& v)
{
  using O = optional<To>;
...
__host__ __device__ constexpr bool
explicit_conversion(Input&& in, const Expect& v)
{
  using O = optional<To>;
