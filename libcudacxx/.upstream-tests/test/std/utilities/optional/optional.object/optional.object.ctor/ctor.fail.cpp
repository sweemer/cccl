...
{
  {
    ...
    cuda::std::optional<NonDestructible>
        o2; // expected-error-re@optional:* {{{{(static_assert|static assertion)}} failed{{.*}}instantiation of optional
            // with a non-destructible type is ill-formed}}
...

  {
    cuda::std::optional< cuda::std::in_place_t>
        o1; // expected-error-re@optional:* {{{{(static_assert|static assertion)}} failed{{.*}}instantiation of optional
            // with in_place_t is ill-formed}}
    cuda::std::optional<const cuda::std::in_place_t>
        o2; // expected-error-re@optional:* {{{{(static_assert|static assertion)}} failed{{.*}}instantiation of optional
            // with in_place_t is ill-formed}}
    cuda::std::optional< volatile cuda::std::in_place_t>
        o3; // expected-error-re@optional:* {{{{(static_assert|static assertion)}} failed{{.*}}instantiation of optional
            // with in_place_t is ill-formed}}
    cuda::std::optional<const volatile cuda::std::in_place_t>
        o4; // expected-error-re@optional:* {{{{(static_assert|static assertion)}} failed{{.*}}instantiation of optional
            // with in_place_t is ill-formed}}
  }
