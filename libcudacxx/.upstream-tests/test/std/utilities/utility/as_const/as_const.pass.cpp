...
operator==(const S& x, const S& y)
{
  return x.i == y.i;
}
__host__ __device__ bool
operator==(const volatile S& x, const volatile S& y)
{
  return x.i == y.i;
}
...
test(T& t)
{
  ...
  static_assert(cuda::std::is_const<
                    typename cuda::std::remove_reference<decltype(cuda::std::as_const<volatile T>(t))>::type>::value,
      "");
  static_assert(
      cuda::std::is_const<
