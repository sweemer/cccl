...
template <class... Ints>
__host__ __device__ constexpr int
constexpr_sum_fn(int x1, Ints... rest)
{
...
enum CallQuals { CQ_None, CQ_LValue, CQ_ConstLValue, CQ_RValue, CQ_ConstRValue };
...
__host__ __device__ void
check_apply_quals_and_types(Tuple&& t)
{
...
{
  template <class... Args>
  __host__ __device__ NothrowMoveable
  operator()(Args...) const noexcept(IsNoexcept)
  {
    return {};
  }
};
...
__host__ __device__ int const&
f(index<3>)
{
  return static_cast<int const&>(my_int);
}
__host__ __device__ int volatile&
f(index<4>)
{
  return static_cast<int volatile&>(my_int);
}
__host__ __device__ int const volatile&
f(index<5>)
{
