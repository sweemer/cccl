...
struct TestLayoutCompatCtors<cuda::std::tuple< Mapping, cuda::std::integer_sequence<size_t, DynamicSizes...>, Mapping2,
...
using test_left_type_pair = cuda::std::tuple< typename cuda::std::layout_left::template mapping<E1>, S1,
    typename cuda::std::layout_left::template mapping<E2>, S2 >;

template <class E1, class S1, class E2, class S2>
using test_right_type_pair = cuda::std::tuple< typename cuda::std::layout_right::template mapping<E1>, S1,
...
struct is_stride_avail< T, RankType,
    cuda::std::enable_if_t< cuda::std::is_same<
        decltype(cuda::std::declval<T>().stride(cuda::std::declval<RankType>())), typename T::index_type >::value > >
    : cuda::std::true_type
{};
