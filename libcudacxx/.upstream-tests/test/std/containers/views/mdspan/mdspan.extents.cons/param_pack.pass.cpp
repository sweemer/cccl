...
check(cuda::std::dextents<size_t, 2> e)
{
...
struct is_param_pack_cons_avail<
    cuda::std::enable_if_t< cuda::std::is_same< decltype(T{cuda::std::declval<IndexTypes>()...}), T >::value >, T,
...
main(int, char**)
{
  ...
  static_assert(
      is_param_pack_cons_avail_v< cuda::std::dextents<int, 1>, my_int_non_nothrow_constructible > == false, "");
