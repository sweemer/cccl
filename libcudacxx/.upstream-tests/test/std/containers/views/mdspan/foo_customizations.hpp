...
struct foo_accessor
{
  ...
  swap(foo_accessor& x, foo_accessor& y)
  {
    x.flag[0] = 99;
    y.flag[0] = 77;
...
public:
  ...
  __MDSPAN_CONDITIONAL_EXPLICIT(
      (!cuda::std::is_convertible<OtherExtents, extents_type>::value)) // needs two () due to comma
  ...
  __MDSPAN_TEMPLATE_REQUIRES(class OtherExtents,
      /* requires */ (
          _LIBCUDACXX_TRAIT(cuda::std::is_constructible, extents_type, OtherExtents) && (extents_type::rank() <= 1)))
  __MDSPAN_CONDITIONAL_EXPLICIT(
      (!cuda::std::is_convertible<OtherExtents, extents_type>::value)) // needs two () due to comma
  __MDSPAN_INLINE_FUNCTION constexpr mapping(
  ...
  __MDSPAN_INLINE_FUNCTION
  constexpr index_type
  stride(rank_type i) const noexcept
  {
    index_type value = 1;
