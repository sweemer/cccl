...
struct _testcases
{
  ...
  __host__ __device__ const cuda::std::complex<double>*
  begin() const
  {
    return &_cases[0];
  }
  ...
  __host__ __device__ cuda::std::complex<double>*
  begin()
  {
    return &_cases[0];
  }
...
__host__ __device__ _testcases
get_testcases()
{
  _testcases tc{cuda::std::complex<double>(1.e-6, 1.e-6),
...
is_about(float x, float y)
{
  assert(cuda::std::abs((x - y) / (x + y)) < 1.e-6);
}

__host__ __device__ void
is_about(double x, double y)
{
