...
testComparisons6(const T& t1, const U& t2, bool isEqual, bool isLess)
{
  if (isEqual) {
    if (!(t1 == t2)) {
      return false;
...
__host__ __device__ TEST_CONSTEXPR_CXX14 bool
testComparisons6Values(Param val1, Param val2)
{
...
template <class T, class U = T>
__host__ __device__ void
AssertComparisons6ConvertibleToBool()
{
  static_assert((cuda::std::is_convertible<decltype(cuda::std::declval<const T&>() == cuda::std::declval<const U&>()),
                    bool>::value),
      "");
  static_assert((cuda::std::is_convertible<decltype(cuda::std::declval<const T&>() != cuda::std::declval<const U&>()),
                    bool>::value),
...
template <class T, class U = T>
__host__ __device__ TEST_CONSTEXPR_CXX14 bool
testComparisons2(const T& t1, const U& t2, bool isEqual)
{
  if (isEqual) {
    if (!(t1 == t2)) {
      return false;
...
template <class T, class Param>
__host__ __device__ TEST_CONSTEXPR_CXX14 bool
testComparisons2Values(Param val1, Param val2)
{
  const bool isEqual = val1 == val2;
...
template <class T, class U = T>
__host__ __device__ void
AssertComparisons2ConvertibleToBool()
{
  static_assert((cuda::std::is_convertible<decltype(cuda::std::declval<const T&>() == cuda::std::declval<const U&>()),
                    bool>::value),
      "");
