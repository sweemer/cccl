...
struct AllocController
{
  ...
  void
  countAlloc(void* p, size_t s, size_t a)
  {
    ++alive;
    ++alloc_count;
    alive_size += s;
  ...
  void
  countDealloc(void* p, size_t s, size_t a)
  {
    --alive;
    ++dealloc_count;
    alive_size -= s;
  ...
  void
  countConstruct(Alloc const&, Tp* p)
  {
    ++construct_called;
...
public:
  bool
  checkAlloc(void* p, size_t s, size_t a) const
  {
    return p == last_alloc_pointer && s == last_alloc_size && a == last_alloc_align;
  }

  bool
  checkAlloc(void* p, size_t s) const
  {
    return p == last_alloc_pointer && s == last_alloc_size;
  }

  bool
  checkAllocAtLeast(void* p, size_t s, size_t a) const
  {
    return p == last_alloc_pointer && s <= last_alloc_size && a <= last_alloc_align;
  }

  bool
  checkAllocAtLeast(void* p, size_t s) const
  {
    return p == last_alloc_pointer && s <= last_alloc_size;
  }

  bool
  checkDealloc(void* p, size_t s, size_t a) const
  {
    return p == last_dealloc_pointer && s == last_dealloc_size && a == last_dealloc_align;
  }

  bool
  checkDealloc(void* p, size_t s) const
  {
    return p == last_dealloc_pointer && s == last_dealloc_size;
  }

  bool
  checkDeallocMatchesAlloc() const
  {
  ...
  template <class... Args, class Alloc, class Tp>
  bool
  checkConstruct(Alloc const&, Tp* p) const
  {
  ...
  template <class Alloc, class Tp>
  bool
  checkDestroy(Alloc const&, Tp* p) const
  {
  ...
  bool
  checkDestroyMatchesConstruct() const
  {
  ...
  bool
  checkIsEqualCalledEq(int n) const
  {
...
inline bool
operator==(CountingAllocator<T, ID> const& x, CountingAllocator<U, ID> const& y)
{
...
inline bool
operator==(MinAlignedAllocator<T> const& x, MinAlignedAllocator<U> const& y)
{
  return &x.getController() == &y.getController();
}
...
template <class T, class U>
inline bool
operator==(NullAllocator<T> const& x, NullAllocator<U> const& y)
{
  return &x.getController() == &y.getController();
}
