...
struct to_chars_test_base
{
  ...
  void
  test(T v, char const (&expect)[N], Ts... args)
  {
    using std::to_chars;
...
private:
  static long long
  fromchars(char const* p, char const* ep, int base, true_type)
  {
    char* last;
  ...
  fromchars(char const* p, char const* ep, int base, false_type)
  {
    char* last;
...
template <typename L1, typename L2>
constexpr auto
concat(L1, L2) -> concat_t<L1, L2>
{
  return {};
}

auto all_signed   = type_list<char, signed char, short, int, long, long long>();
auto all_unsigned = type_list<unsigned char, unsigned short, unsigned int, unsigned long, unsigned long long>();
