...
#define TEST_SUITE(Name)                                    \
  namespace Name {                                          \
  inline ::rapid_cxx_test::test_suite&                      \
  get_test_suite()                                          \
  {                                                         \
...
#  define TEST_CASE(Name)                                                                                       \
    ...
    static ::rapid_cxx_test::registrar RAPID_CXX_TEST_PP_CAT(rapid_cxx_test_registrar_, Name)(get_test_suite(), \
...
#  define TEST_CASE(Name)                                                                                        \
    ...
    _Pragma("clang diagnostic push")                                                                             \
        _Pragma("clang diagnostic ignored \"-Wglobal-constructors\"") static ::rapid_cxx_test::registrar         \
            RAPID_CXX_TEST_PP_CAT(rapid_cxx_test_registrar_, Name)(get_test_suite(),                             \
                ::rapid_cxx_test::test_case(__FILE__, #Name, __LINE__, &RAPID_CXX_TEST_PP_CAT(Name, _invoker))); \
...
struct failure_type
{
  enum enum_type { none, unsupported, warn, check, require, assert, uncaught_exception };
};
...
struct test_outcome
{
  ...
  test_outcome(failure_type_t type1, const char* file1, const char* func1, std::size_t line1, const char* expression1,
      const char* message1)
      : type(type1)
      , file(file1)
      , func(func1)
...
////////////////////////////////////////////////////////////////////////////
inline void
set_checkpoint(const char* file, const char* func, std::size_t line)
{
...
#  pragma GCC diagnostic pop
  ...
  std::size_t
  test_case_count() const
  {
    return m_testcases;
  }

  std::size_t
  test_case_failure_count() const
  {
    return m_testcase_failures;
  }

  std::size_t
  unsupported_count() const
  {
    return m_unsupported;
  }

  std::size_t
  assertion_count() const
  {
    return m_assertions;
  }

  std::size_t
  warning_failure_count() const
  {
    return m_warning_failures;
  }

  std::size_t
  check_failure_count() const
  {
    return m_check_failures;
  }

  std::size_t
  require_failure_count() const
  {
  ...
  void
  print_summary(const char* suitename) const
  {
...
private:
  ...
  void
  report_error(test_outcome o) const
  {
    std::fprintf(stderr,
...
bool
check_equal_collections_impl(Iter1 start1, Iter1 const end1, Iter2 start2, Iter2 const end2)
{
