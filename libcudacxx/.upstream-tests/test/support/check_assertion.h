...
struct AssertionInfoMatcher
{
  ...
  bool
  Matches(char const* file, int line, char const* message) const
  {
...
private:
  bool
  CheckLineMatches(int got_line) const
  {
    if (line_ == any_line) {
      return true;
    }
  ...
  bool
  CheckFileMatches(std::string_view got_file) const
  {
    assert(!empty() && "empty matcher");
    if (file_ == any_file) {
      return true;
  ...
  bool
  CheckMessageMatches(std::string_view got_msg) const
  {
    assert(!empty() && "empty matcher");
    if (msg_ == any_msg) {
      return true;
...
struct DeathTest
{
  enum ResultKind { RK_DidNotDie, RK_MatchFound, RK_MatchFailure, RK_SetupFailure, RK_Unknown };

  static const char*
  ResultKindToString(ResultKind RK)
  {
#define CASE(K) \
case K:         \
  return #K
  ...
  int
  getChildExitCode() const
  {
    return exit_code_;
  }
  std::string const&
  getChildStdOut() const
  {
    return stdout_from_child_;
  }
  std::string const&
  getChildStdErr() const
  {
...
private:
  ...
  int
  GetStdOutReadFD() const
  {
    return stdout_pipe_fd_[0];
  }

  int
  GetStdOutWriteFD() const
  {
    return stdout_pipe_fd_[1];
  }

  int
  GetStdErrReadFD() const
  {
    return stderr_pipe_fd_[0];
  }

  int
  GetStdErrWriteFD() const
  {
...
void
std::__libcpp_verbose_abort(char const* format, ...)
{
...
template <class Func>
inline bool
ExpectDeath(const char* stmt, Func&& func, AssertionInfoMatcher Matcher)
{
...
template <class Func>
inline bool
ExpectDeath(const char* stmt, Func&& func)
{
