...
static const fs::path RecDirIterationList[] = {makePath("dir1/dir2"),
    makePath("dir1/file1"),
    makePath("dir1/file2"),
    makePath("dir1/dir2/afile3"),
    makePath("dir1/dir2/dir3"),
...
struct scoped_test_env
{
  ...
  // 2GB.
  std::string
  create_file(std::string filename, uintmax_t size = 0)
  {
#  if defined(__LP64__)
...
#  endif
  ...
  std::string
  create_dir(std::string filename)
  {
    filename = sanitize_path(std::move(filename));
  ...
  std::string
  create_symlink(std::string source, std::string to)
  {
  ...
  std::string
  create_hardlink(std::string source, std::string to)
  {
    source = sanitize_path(std::move(source));
    to     = sanitize_path(std::move(to));
...
private:
  ...
  make_arg(std::string const& arg)
  {
    return "'" + arg + "'";
  }

  static inline std::string
  make_arg(std::size_t arg)
  {
  ...
  static inline void
  fs_helper_run(std::string const& raw_cmd)
  {
...
const MultiStringType PathList[] = {MKSTR(""),
    MKSTR(" "),
    MKSTR("//"),
    MKSTR("."),
    MKSTR(".."),
...
inline void
PathReserve(fs::path& p, std::size_t N)
{
...
bool
checkCollectionsEqual(Iter1 start1, Iter1 const end1, Iter2 start2, Iter2 const end2)
{
...
template <class Iter1, class Iter2>
bool
checkCollectionsEqualBackwards(Iter1 const start1, Iter1 end1, Iter2 const start2, Iter2 end2)
{
  while (start1 != end1 && start2 != end2) {
    --end1;
    --end2;
...
inline bool
ErrorIsImp(const std::error_code& ec, std::vector<std::errc> const& errors)
{
...
inline bool
ErrorIs(const std::error_code& ec, std::errc First, ErrcT... Rest)
{
...
void
SleepFor(std::chrono::seconds dur)
{
...
inline bool
PathEq(fs::path const& LHS, fs::path const& RHS)
{
...
struct ExceptionChecker
{
  ...
  void
  operator()(fs::filesystem_error const& Err)
  {
  ...
  void
  check_libcxx_string(fs::filesystem_error const& Err)
  {
