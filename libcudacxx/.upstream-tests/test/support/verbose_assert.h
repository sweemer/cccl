...
template <class Tp,
    int ST =
        (IsStreamable<decltype(std::cerr), Tp>::value ? 1 : (IsStreamable<decltype(std::wcerr), Tp>::value ? 2 : -1))>
struct SelectStream
{
...
struct AssertData
{
  ...
  AssertData&
  SetFailed(std::string xmsg = std::string())
  {
    msg    = xmsg;
    passed = false;
    return *this;
...
public:
  ...
  class LogType
  {
    ...
    template <class Tp>
    friend LogType&
    operator<<(LogType& log, Tp const& value)
    {
      if (!log.is_disabled) {
    ...
    friend LogType&
    operator<<(LogType& log, EndLType* m)
    {
      if (!log.is_disabled) {
...
template <class It1>
std::string
PrintRange(const char* Name, It1 F, It1 E)
{
  std::stringstream ss;
  ss << "  " << Name << " = [";
  while (F != E) {
...
std::string
PrintMismatch(Tp const& LHS, Up const& RHS, int Elem)
{
  std::stringstream ss;
...
struct EqualToComp
{
  template <class Tp, class Up>
  bool
  operator()(Tp const& LHS, Up const& RHS) const
  {
    return LHS == RHS;
  }
};
...
AssertData
CheckCollectionsEqual(It1 F1, It1 E1, It2 F2, It2 E2, AssertData Data, Comp C = EqualToComp())
{
  const It1 F1Orig = F1;
  const It2 F2Orig = F2;
  bool Failed      = false;
...
#define ASSERT_COLLECTION_EQ_COMP(F1, E1, F2, E2, Comp)                                                           \
  (::verbose_assert::AssertHandler(                                                                               \
      ::verbose_assert::CheckCollectionsEqual(F1,                                                                 \
          E1,                                                                                                     \
          F2,                                                                                                     \
          E2,                                                                                                     \
