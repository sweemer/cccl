...
struct ContainerDebugHelper<Container, std::pair<const Key, Value> >
{
  ...
  static ValueType
  makeValueType(int key = 0, int val = 0)
  {
...
struct BasicContainerChecks
{
  ...
  static_assert(
      std::is_same<test_allocator<value_type>, allocator_type>::value, "the container must use a test allocator");
...
public:
  ...
  static Container
  makeContainer(int size, allocator_type A = allocator_type())
  {
    Container C(A);
    if constexpr (CT == CT_ForwardList) {
