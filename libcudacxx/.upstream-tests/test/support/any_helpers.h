...
template <class Type>
constexpr auto
has_value_member(int) -> decltype(std::declval<Type&>().value, true)
{
  return true;
}
template <class>
constexpr bool
has_value_member(long)
{
  return false;
}
...
template <class Type>
std::enable_if_t<has_value_member<Type>(0)> _LIBCUDACXX_AVAILABILITY_THROW_BAD_ANY_CAST
...
// 'value'.
template <class Type>
_LIBCUDACXX_AVAILABILITY_THROW_BAD_ANY_CAST void
modifyValue(std::any& a, int value)
{
  using namespace std;
...
void
assertArgsMatch(std::any const& a)
{
  using namespace std;
  using namespace std::experimental;
  assert(a.has_value());
