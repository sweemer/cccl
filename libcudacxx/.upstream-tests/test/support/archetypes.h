...
protected:
  ...
  __host__ __device__ TestBase&
  operator=(TestBase const& o) noexcept
  {
    assert(o.value != -1);
    assert(o.value != -999);
    ++assigned();
...
protected:
#endif // !TEST_WORKAROUND_C1XX_BROKEN_ZA_CTOR_CHECK
  __host__ __device__ constexpr static int
  check_value(int const& val)
  {
#if TEST_STD_VER < 14
  ...
  __host__ __device__ constexpr static int
  check_value(int& val, int val_cp = 0)
  {
#if TEST_STD_VER < 14
...
operator==(Tp const& L, Tp const& R) noexcept
{
