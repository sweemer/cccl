...
public:
  ...
  friend bool
  operator==(const non_default_test_allocator& x, const non_default_test_allocator& y)
  {
    return x.data_ == y.data_;
  }
  friend bool
...
public:
  ...
  friend bool
  operator==(const other_allocator& x, const other_allocator& y)
  {
    return x.data_ == y.data_;
  }
  friend bool
...
struct limited_alloc_handle
{
  ...
  void
  deallocate(void* ptr, std::size_t N)
  {
    if (ptr == last_alloc_) {
      last_alloc_ = nullptr;
...
inline bool
operator==(limited_allocator<T, N> const& LHS, limited_allocator<U, N> const& RHS)
{
