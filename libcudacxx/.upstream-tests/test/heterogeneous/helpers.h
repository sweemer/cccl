//===----------------------------------------------------------------------===//
//
// Part of the libcu++ Project, under the Apache License v2.0 with LLVM
...
__host__ inline std::vector<std::thread>&
host_threads()
{
...
__host__ inline std::vector<cudaStream_t>&
device_streams()
{
...
validate_impl(T& object) -> decltype(Tester::validate(object), void())
{
...
__global__ void
construct_kernel(void* address, Args... args)
{
  new (address) T(args...);
}
...
template <typename Tester, typename T>
__global__ void
initialization_kernel(T& object)
{
...
template <typename T, typename... Args>
T*
device_construct(void* address, Args... args)
{
...
void
validate_device_dynamic(tester_list<Testers...>, Args... args)
{
  void* pointer;
...
template <typename T, typename... Testers, typename... Args>
void
validate_managed(tester_list<Testers...>, Args... args)
{
  ...
  // The managed variable template part of this test is disabled under clang,
  // pending nvbug 2790305 being fixed.
...
bool
check_managed_memory_support(bool is_async)
{
...
void
validate_not_movable(Args... args)
{
...
enum class performer_side { initialize, validate };
...
struct validate_list<true, performer_list<First, Rest...>>
{
  using type = generate_variants<
      ...
      // testing for `ii` also covers `vv`, and testing for `iv` also covers
      // `vi`. Therefore, only the first half of the generated sequence is
      // actually meaningfully useful.
      tester_list<performer_adapter<First, performer_side::initialize>>, Rest...>;
};
